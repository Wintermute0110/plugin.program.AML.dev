...
class Main:
    ...
    def run_plugin(self):
        ...
        if args_size == 0:
            ...
            # >> MAME Favourites
                ...
                machine = args['machine'][0] if 'machine' in args else ''
                self._command_context_manage_mame_fav(machine)
    ...
    def _render_root_list(self):
        ...
        if self.settings['display_MAME_favs']:
            ...
            CM_URL = URL_manage = self._misc_url_1_arg_RunPlugin('command', 'MANAGE_MAME_FAV')
            self._render_root_list_row_custom_CM('<Favourite MAME machines>', 
                                                 self._misc_url_1_arg('command', 'SHOW_MAME_FAVS'),
                                                 CM_title, CM_URL)
        if self.settings['display_SL_favs']:
            self._render_root_list_row_standard('<Favourite Software Lists ROMs>',
                                                self._misc_url_1_arg('command', 'SHOW_SL_FAVS'))
        if self.settings['display_custom_filters']:
            self._render_root_custom_filter_row('[Custom MAME filters]',
                                                self._misc_url_1_arg('command', 'SHOW_CUSTOM_FILTERS'))

        CM_title = 'Manage Most Played machines'
        CM_URL = URL_manage = self._misc_url_1_arg_RunPlugin('command', 'MANAGE_MAME_MOST_PLAYED')
        self._render_root_list_row_custom_CM('{Most played MAME machines}',
                                             self._misc_url_1_arg('command', 'SHOW_MAME_MOST_PLAYED'),
                                             CM_title, CM_URL)

        CM_title = 'Manage Recently Played machines'
        CM_URL = URL_manage = self._misc_url_1_arg_RunPlugin('command', 'MANAGE_MAME_RECENT_PLAYED')
        self._render_root_list_row_custom_CM('{Recently played MAME machines}',
                                             self._misc_url_1_arg('command', 'SHOW_MAME_RECENTLY_PLAYED'),
                                             CM_title, CM_URL)
    ...
    def _render_root_list_row_custom_CM(self, root_name, root_URL, CM_title, CM_URL):
        # --- Create listitem row ---
        ICON_OVERLAY = 6
        listitem = xbmcgui.ListItem(root_name)
        listitem.setInfo('video', {'title' : root_name, 'overlay' : ICON_OVERLAY})
    ...
    def _command_context_view(self, machine_name, SL_name, SL_ROM, location):
        ...
        # --- View database information and statistics stored in control dictionary ---
            ...
            # --- Scanner statistics ---
                ...
                self._display_text_window('Scanner statistics', '\n'.join(info_text))
            ...
            # --- Audit statistics ---
                ...
                self._display_text_window('Database information and statistics', '\n'.join(info_text))
        ...
        # --- View audit reports ---
            ...
            if type_sub == 0:
                ...
                with open(PATHS.REPORT_MAME_AUDIT_FULL_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('MAME audit report (Full)', myfile.read())
                ...
                with open(PATHS.REPORT_MAME_AUDIT_GOOD_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('MAME audit report (Good)', myfile.read())
                ...
                with open(PATHS.REPORT_MAME_AUDIT_ERRORS_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('MAME audit report (Errors)', myfile.read())
                ...
                with open(PATHS.REPORT_MAME_AUDIT_ROM_GOOD_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('MAME audit report (ROM Good)', myfile.read())
                ...
                with open(PATHS.REPORT_MAME_AUDIT_ROM_ERRORS_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('MAME audit report (ROM Errors)', myfile.read())
                ...
                with open(PATHS.REPORT_MAME_AUDIT_CHD_GOOD_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('MAME audit report (CHD Good)', myfile.read())
                ...
                with open(PATHS.REPORT_MAME_AUDIT_CHD_ERRORS_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('MAME audit report (CHD Errors)', myfile.read())
            ...
            # >> SL audit reports
                ...
                with open(PATHS.REPORT_SL_AUDIT_FULL_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('SL audit report (Full)', myfile.read())
                ...
                with open(PATHS.REPORT_SL_AUDIT_GOOD_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('SL audit report (Good)', myfile.read())
                ...
                with open(PATHS.REPORT_SL_AUDIT_ERRORS_PATH.getPath(), 'r') as myfile:
                    self._display_text_window('SL audit report (Errors)', myfile.read())
    ...
    def _render_fav_machine_row(self, m_name, machine, m_assets, location):
        ...
        if location == LOCATION_MAME_FAVS:
            ...
            URL_manage = self._misc_url_2_arg_RunPlugin('command', 'MANAGE_MAME_MOST_PLAYED', 'machine', m_name)
            commands = [
                ('Info / Utils',  URL_view_DAT),
                ('View / Audit',  URL_view),
            ...
            URL_manage = self._misc_url_2_arg_RunPlugin('command', 'MANAGE_MAME_RECENT_PLAYED', 'machine', m_name)
            commands = [
                ('Info / Utils',  URL_view_DAT),
                ('View / Audit',  URL_view),
    ...
    def _command_context_manage_mame_most_played(self, machine):
    ...
    def _command_context_manage_mame_recent_played(self, machine):
        kodi_dialog_OK('Not finished yet.')
        return
    ...
    def _command_check_AML_configuration(self):
        ...
        # ROM path
        check_dir_ERR(slist, self.settings['rom_path'], 'MAME ROM path')
        slist.append('')
        ...
        check_dir_WARN(slist, self.settings['chd_path'], 'MAME CHD path')

        # --- Samples path ---
        check_dir_WARN(slist, self.settings['samples_path'], 'MAME Samples path')
        slist.append('')

        # --- Software Lists paths ---
        slist.append('[COLOR orange]Software List paths[/COLOR]')
        check_dir_WARN(slist, self.settings['SL_hash_path'], 'SL hash path')
        check_dir_WARN(slist, self.settings['SL_rom_path'], 'SL ROM path')
        check_dir_WARN(slist, self.settings['SL_chd_path'], 'SL CHD path')
        slist.append('')
        ...
        check_file_WARN(slist, self.settings['catver_path'], 'Catver.ini file')
        check_file_WARN(slist, self.settings['catlist_path'], 'Catlist.ini file')
        check_file_WARN(slist, self.settings['genre_path'], 'Genre.ini file')
        check_file_WARN(slist, self.settings['nplayers_path'], 'NPlayers.ini file')
        check_file_WARN(slist, self.settings['bestgames_path'], 'bestgames.ini file')
        check_file_WARN(slist, self.settings['series_path'], 'series.ini file')
        slist.append('')

        # --- Optional DAT files ---
        slist.append('[COLOR orange]DAT files[/COLOR]')
        check_file_WARN(slist, self.settings['history_path'], 'History.dat file')
        check_file_WARN(slist, self.settings['mameinfo_path'], 'MameINFO.dat file')
        check_file_WARN(slist, self.settings['gameinit_path'], 'Gameinit.dat file')
        check_file_WARN(slist, self.settings['command_path'], 'Command.dat file')
